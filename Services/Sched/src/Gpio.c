...
Gpio_enuErrorStatus Gpio_enuInitPin( GpioPinCfg_t * Add_strPinCfg )
{
	...
	else
	{
		...
		/* MODER */
		Loc_u32Mask = GPIO_u32MODER_MSK << ( Gpio_u8PinNumber * GPIO_u8DOUBLE_SHIFT );
		Loc_u32Mode = ( Add_strPinCfg -> Mode ) << ( Gpio_u8PinNumber * GPIO_u8DOUBLE_SHIFT );

		Loc_u32Reg = ( ( Gpio_t * const ) ( Add_strPinCfg -> Port ) ) -> MODER ;
		Loc_u32Reg |= Loc_u32Mask;
		Loc_u32Reg &= ( ~Loc_u32Mask | Loc_u32Mode );
		( ( Gpio_t * const ) ( Add_strPinCfg -> Port) ) -> MODER = Loc_u32Reg ;

		/* OTYPER */
		Loc_u32Mask = ( ( GPIO_u32OTYPER_MSK >> GPIO_u8OTYPER_SHIFT ) << Gpio_u8PinNumber  );
		Loc_u32Mode = ( ( ( Add_strPinCfg -> Mode ) >> GPIO_u8OTYPER_SHIFT ) << Gpio_u8PinNumber );
		...
		/* PUPDR */
		Loc_u32Mask = ( ( GPIO_u32PUPDR_MSK >> GPIO_u8PUPDR_SHIFT ) << (Gpio_u8PinNumber * GPIO_u8DOUBLE_SHIFT )  );
		Loc_u32Mode = ( ( ( Add_strPinCfg -> Mode ) >> GPIO_u8PUPDR_SHIFT ) << (Gpio_u8PinNumber * GPIO_u8DOUBLE_SHIFT ) );
		...
		/* OSPEEDR */
		Loc_u32Mask = GPIO_u32OSPEEDR_MSK << ( Gpio_u8PinNumber * GPIO_u8DOUBLE_SHIFT );
		Loc_u32Mode = ( Add_strPinCfg -> Speed ) << ( Gpio_u8PinNumber * GPIO_u8DOUBLE_SHIFT );
