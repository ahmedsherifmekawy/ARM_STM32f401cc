...
{
	Gpio_enuErrorStatus Loc_enuError = Ok;
	u32 Loc_u32Reg;
	u32 Loc_u32Msk;
	if( Copy_u32Port > GPIOH_BASE_ADDRESS || Copy_u32Port < GPIOA_BASE_ADDRESS )
	{
	...
	else
	{
		...
		if( Copy_u8AF_Number < 8 )
		{
			Loc_u32Msk = GPIO_u32AF_MSK << Copy_u8AF_Number ;
			Loc_u32Reg = ( ( Gpio_t * const ) Copy_u32Port ) -> AFRL ;
			Loc_u32Reg &= ~Loc_u32Msk;
			Loc_u32Reg |= (  Loc_u32Msk & ( (u32) Copy_u8AF_Value << Copy_u8AF_Number ) );
			( ( Gpio_t * const ) Copy_u32Port ) -> AFRL = Loc_u32Reg;
		}
		else
		{
			Copy_u8AF_Number -= 8;
			Loc_u32Msk = GPIO_u32AF_MSK << Copy_u8AF_Number ;
			Loc_u32Reg = ( ( Gpio_t * const ) Copy_u32Port ) -> AFRH ;
			Loc_u32Reg &= ~Loc_u32Msk;
			Loc_u32Reg |= (  Loc_u32Msk & ( (u32) Copy_u8AF_Value << Copy_u8AF_Number ) );
			( ( Gpio_t * const ) Copy_u32Port ) -> AFRH = Loc_u32Reg;
		}
	}
